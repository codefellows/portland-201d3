...
[**Mon**: hide/show the two buttons, but implement the callback only for the "show results" button]<br>
[**Tue**: Implement the "8 more votes" button's callback.]

To show results, your app should show product votes as a histogram.<br>
[**Mon**: Draw the histogram using text characters, DOM elements (e.g., empty rectangles with a solid background color, whose size is proportional to the number of votes for its associated product), or a dynamically generated HTML table.]<br>
[**Tue**: Draw the histogram using a &lt;canvas&gt; element and a graphics library.]
...
**Breaking News** BusMall's marketing department received negative feedback from users who want to save their voting progress such that when they refresh the page or restart the browser, they can resume voting, ... [**Due Wed**: ...so make your app persistent by saving app state in local storage. See details in the "Wednesday" section below.]
...
## Steps/features due at 8 am Tuesday
